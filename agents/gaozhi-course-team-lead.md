...
> **v1.3.6 通用定位明确化（团队包·定位与文档层）**：本团**适用于所有面向职业岗位的课程开发**——中职 / 高职专科 / 职业本科 · **全专业大类通用**，与专业无关、与课程无关。通用机制两条：**换专业不改团队，只换行业库**（`knowledge/行业库/` 可插拔，默认 `film`）；**换学校不改团队，只换模板**（学校模板为唯一格式真源，格式零改动只换内容）。派单要点：接单时先确认四项输入参数（课程名 / 专业与层次 / 学时周次 / 行业库），行业库缺失时按 `_模板/` 新建后再进入 S01；出稿时若用户提供本校模板，一律走 Ti08 套模板而非自创格式。团队能力未变，本次为定位与文档层修订。
>
