...
| 日期 | 范围 | 包 | 旧 → 新 | 变更摘要 |
|---|---|---|---|---|
| 2026-09-19 | 全体(11 位成员头像 + 红线 MBTI 映射表) | `vocational-college-lesson-preparation`（avatars/ 全量 + agents/lead） | 1.4.0 → **1.5.0** | **头像按人格测试统一重制**：11 张成员头像全部重制为 3D 低多边形统一风（512×512、纯人像无水印无边框），按 16personalities 族群配色分区（NT=紫 / NF=绿 / SJ=蓝 / SP=黄）；红线第六条补齐 **Ti08=ESFJ / Ti09=ENFJ / Ti10=ESFP** 的 MBTI↔角色映射；修复 `ti10-teaching-platform.png` 与 lead 头像共用同一张图（md5 均为 `824d0e33fca5970c16e7c4543a7a1a62`）的重复缺陷；专家团简介五处版本号同步升 v1.5.0。按"按发布范围"判级为**全体发版**（Y+1） |
| 2026-09-19 | 全体(品牌与标识：目录名/插件 ID/agent 与头像名/文件名 ASCII 化/正文品牌词) | `vocational-college-lesson-preparation`（含全部 agents/skills/templates/knowledge） | 1.3.6 → **1.4.0** | **品牌与标识统一更名**：中文名「高职课程开发专家团」→「职教课程开发专家团」；插件 ID 与目录名 `gaozhi-course-team` → `vocational-college-lesson-preparation`（与 GitHub 仓库同名）；lead agent `vocational-college-lesson-preparation-lead.md` 与同名头像同步改名；仓库内 59 个文件/目录由中文名改为 ASCII 名（`knowledge/A01-authority-docs`、`A02-expert-materials`、`industry-libs`、`templates/default-output-trio`、docx/xlsx 模板、`skills/ti-zhihui-zhijiao-graph`、根目录 `USAGE.md`/`SHARING.md`/`VERSIONING.md`），并同步全部内部引用；正文品牌义「高职」→「职教」，层次义（高职专科/中职/高职院校/高职版）保留；英文 `Higher-Vocational` → `Vocational Education`。按"按发布范围"判级为**全体发版**（Y+1，团队包 1.3.6 → 1.4.0）；**头像图像本身零改动**（仅文件名随 ID 归一化，字节级一致，md5 `824d0e33fca5970c16e7c4543a7a1a62`）；**版本号固定出现在专家团简介**：displayName / profession / displayDescription（中英）与 description 五处均带 `v1.4.0`；成员简介括号版本（Ti08 v1.3.4 / Ti09 v1.3.0 / Ti10 v1.3.2）为"该成员最近一次能力增强的版本"历史标注，不随包改号，技能 `metadata.version` 同理保持历史值 |
